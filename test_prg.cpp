...
/*
* @Author: Ganesh Joshi
* CSCI 611 Project 3 GoldRush Enhancement
* Date : 04/04/2015
*/
...
//string mq_name="/GJ_player2_mq";
//string mq_name1="/GJ_player2_mq";
...
void writeMessage(std::string message,int player){
  ...
  if(player == G_PLR0)
    mq_name="/GJ_player0_mq";
  else if(player == G_PLR1)
    mq_name="/GJ_player1_mq";
  else if(player == G_PLR2)
    mq_name="/GJ_player2_mq";
  else if(player == G_PLR3)
    mq_name="/GJ_player3_mq";
...
void broadCast(std::string message,int player){
    ...
    if(player & G_PLR0) sendmessage("/GJ_player0_mq",message);
    if(player & G_PLR1) sendmessage("/GJ_player1_mq",message);
    if(player & G_PLR2) sendmessage("/GJ_player2_mq",message);
    if(player & G_PLR3) sendmessage("/GJ_player3_mq",message);
}
...
int main()
{
  ...
  shared_mem=shm_open("/sharedMemoryGJ",O_RDWR,S_IRUSR|S_IWUSR);
  if(shared_mem == -1){
...
mapstruct * gameinit(sem_t* lock,bool isNewGame, int &loc,int shared_mem,int &current_player){
  ...
  shared_mem=shm_open("/sharedMemoryGJ",O_RDWR|O_CREAT,S_IRUSR|S_IWUSR);
